...
impl<'a> Tokenizer2<'a> {
    pub fn new_tokenizer(tokenizer: Tokenizer<'a>) -> Self {
        Self {
            tokenizer: Cell::new(TokenizerKind::Tokenizer(tokenizer)),
    ...
    pub fn enter_macro(&self, stream: (Location<'a>, Location<'a>)) {
        ...
        self.tokenizer
            .set(TokenizerKind::MacroTokenizer(MacroTokenizer2::new(
                (tokenizer, stream.1),
                ret,
            )));
    ...
    pub fn leave_macro(&self) {
        ...
        self.tokenizer
            .set(TokenizerKind::Tokenizer(tokenizer.tokenizer));
